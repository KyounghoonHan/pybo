...
def index(request):
    ...
    if so == "recommend":
        ...
        question_list = Question.objects.filter(category="qna").annotate(num_answer=Count("answer")).order_by("-num_answer", "-create_date")
    else: # recent part
        question_list = Question.objects.filter(category="qna").order_by("-create_date")
...
def board(request):
    ...
    # 조회
    if so == "recommend":
        question_list = Question.objects.filter(category="board").annotate(num_voter=Count("voter")).order_by("-num_voter", "-create_date")
    elif so == "popular":
        question_list = Question.objects.filter(category="board").annotate(num_answer=Count("answer")).order_by("-num_answer", "-create_date")
    else: # recent part
        question_list = Question.objects.filter(category="board").order_by("-create_date")
    # question_list = Question.objects.order_by("-create_date")
